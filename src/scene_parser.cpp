...
Light *SceneParser::parsePointLight() {
    ...
    Vector3f color = readVector3f();
    getToken(token);
    assert (!strcmp(token, "brightness"));
    double brightness = readFloat();
    getToken(token);
    assert (!strcmp(token, "}"));
    return new PointLight(position, color, brightness);
}
// ====================================================================
// ====================================================================
...
Material *SceneParser::parseMaterial() {
    ...
    auto *answer = new Material(diffuseColor, specularColor, emissionColor, shininess, type, tt, filename, refl, diff, spec, refr, color);
    return answer;
}
...
RevSurface *SceneParser::parseRevSurface() {
    ...
    getToken(token);
    assert (!strcmp(token, "}"));
    auto *answer = new RevSurface(profile, current_material, pos);
    return answer;
}
...
Transform *SceneParser::parseTransform() {
    ...
    getToken(token);
    assert (!strcmp(token, "}"));
    return new Transform(matrix, object, s, t);
}
